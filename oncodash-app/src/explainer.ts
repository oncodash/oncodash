...
function drawLinks(
    ...
    for (const link of links) {
        ...
        const shape = `M 0 0 v ${h} h ${a} l ${h / 2} ${b} l ${b} ${b} h ${-1 * a}`;
        ...
        svg.append('path')
          ...
          .attr('filter', `url(#blur)-${Math.round((1 - link.certainty)*10 / 10)})`)
    }
}
...
function drawGraph(graphData: NodeLink): HTMLElement {
    ...
    for (const i of Object.keys(colData)) {
        ...
        for (const k of Object.keys(colBounds)) {
            ...
            linkData[colData[+i].colname] = {links: colLinks, bound: colBounds};
        }
    }
