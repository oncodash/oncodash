...
def query_oncokb_cnas(cnas: [cna_annotation]):
    ...
    data = [
        {
            ...
            "copyNameAlterationType": f"{cnas[i].alteration}",
            "gene": {
                "hugoSymbol": f"{cnas[i].hugoSymbol}"
            },
            "tumorType": f"{cnas[i].tumorType}"
        }
    ...
    if (response.status == 200):
        ...
        for rjson in respjson:
            ...
            cna_id = idsplit[2]
            rec, created = cna_annotation.objects.update_or_create(
