...
def main(
        **kwargs):  # clinical_data: pd.DataFrame, cna_data: pd.DataFrame, snv_data: pd.DataFrame, ascat_data: pd.DataFrame, amis_data: pd.DataFrame):
    ...
    if kwargs["copy_number_alterations"]:
        ...
        cores = int(kwargs["cores"]) if kwargs["cores"] else 1
        cna_data = pd.read_csv(kwargs.get("copy_number_alterations", ""), sep="\t")

        refgenome = kwargs["refgen"] if kwargs["refgen"] else "GRCh38"
        ploidy_coeff = float(kwargs["ploidy_threshold"]) if kwargs["ploidy_threshold"] else 2.5
        ...
        header = ["ID", "Gene", "chr", "start", "end", "strand", "band", "type", "sample", "nProbesCr", "nProbesAf",
                  "logR", "baf", "nAraw", "nBraw", "nMajor", "nMinor", "purifiedLogR", "purifiedBaf", "purifiedLoh",
                  "CNstatus", "LOHstatus", "minPurifiedLogR", "maxPurifiedLogR", "breaksInGene"]

        cnaobjects = []

        ascats = pd.read_csv(kwargs["ascatestimates"], sep="\t", encoding='utf-8')
        cnas_filtered = df_apply(cna_data, filter_cnas_by_ploidy, None, None, True, False, cores=cores, ascats=ascats,
                                 ploidy_coeff=ploidy_coeff).dropna()
    ...
    if kwargs["somatic_variants"] and kwargs["cn_annotations"] and kwargs["pid"]:
        snv_file = str(kwargs["somatic_variants"]) + "/" + str(kwargs["pid"]) + ".csv"
        output = kwargs["output"]
        ...
        # cnas = pd.read_csv(kwargs["cn_annotations"], sep="\t", encoding='utf-8'
        # TODO: read by patient CNAs
        cnas = dd.read_csv(str(kwargs["cn_annotations"]) + "/" + str(kwargs["pid"]) + "*.csv", sep="\t").compute().dropna(
            subset=['Gene', 'nMinor', 'nMajor', 'LOHstatus'])
        print(cnas)
        samples = cnas['sample'].drop_duplicates()
        print(samples)
        snvs_filtered = df_apply(snvs, filter_and_classify_snvs, None, None, True, True, cores=cores, cnas=cnas,
                                 ascats=ascats, samples=samples)
    ...
    if kwargs["somatic_variants"] and kwargs["post_annotate"]:
        ...
        cnas = pd.read_csv(kwargs["cn_annotations"], sep="\t", encoding='utf-8')[
            ['sample', 'Gene', 'nMinor', 'nMajor', 'LOHstatus']].dropna(
            subset=['Gene', 'nMinor', 'nMajor', 'LOHstatus'])
        ...
        for gname, cna_grp in cna_grps:
            ...
            try:
                ...
                snvs_filtered = df_apply(snv_grp, post_filter_and_classify_snvs_by_sample, None, None, True, True,
                                         cores=cores,
